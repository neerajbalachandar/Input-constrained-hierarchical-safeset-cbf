...
def evaluate_cbf(state, obs, l_shift):
    ...
    eps = 1e-5
    norm_p = np.linalg.norm(p_rel) + eps
    norm_v = np.linalg.norm(v_rel) + eps

    cone_base = np.sqrt(max(0, norm_p**2 - (orad + 0.4)**2)) + eps
    cos_phi = cone_base / norm_p

    h_val = np.dot(p_rel, v_rel) + norm_p * norm_v * cos_phi
    return h_val
...
def solve_cbf_qp(nom_a, nom_alpha, state, obs, gamma, dt, l_shift):
    ...
    soy = ca.MX.sym('soy')
    sov = ca.MX.sym('sov')
    soyaw = ca.MX.sym('soyaw')
    sor = ca.MX.sym('sor')

    sys_params = ca.vertcat(sx, sy, syaw, sv, sw, sox, soy, sov, soyaw, sor)

    opt_a = ca.MX.sym('opt_a')                                              
    opt_alpha = ca.MX.sym('opt_alpha')
    ...
    f_eps = 1e-5                                                          
    mag_dp = ca.norm_2(dp) + f_eps
    mag_dv = ca.norm_2(dv) + f_eps

    cone_edge = ca.sqrt(mag_dp**2 - (sor + 0.4)**2 + f_eps)           
    angle_cos = cone_edge / mag_dp

    h_function = ca.dot(dp, dv) + mag_dp * mag_dv * angle_cos    
    ...
    qp_def = {'x': controls, 'f': cost, 'g': ca.vertcat(*safety_constraint), 'p': sys_params}    
    ...
    qp_solver = ca.qpsol('cbf_qp_solver', 'qrqp', qp_def, solver_opts)

    current_vals = [state['x'], state['y'], state['yaw'], state['v'], state['w'],              
                    obs['x'], obs['y'], obs['v'], obs['yaw'], obs['r']]

    try:                                                                    
        solution = qp_solver(x0=[nom_a, nom_alpha], lbg=0, ubg=ca.inf, p=current_vals)
