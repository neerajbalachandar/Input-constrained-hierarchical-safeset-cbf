...
yalmip('clear')
%% settings
if ~exist('save_file','var'); save_file = 0; end
if ~exist('print_file','var'); print_file = 1; end
if ~exist('linear_like_form','var'); linear_like_form = 2; end
if ~exist('include_input_limits','var'); include_input_limits = 1; end
if ~exist('deg_X','var'); deg_X = 0; end
if ~exist('deg_Y','var'); deg_Y = 2; end
if ~exist('X_states_index','var'); X_states_index = 2; end
if ~exist('Y_states_index','var'); Y_states_index = [1 2]; end

% wrapper options
if ~exist('run_refinement','var'); run_refinement = 0; end
if ~exist('make_plot','var'); make_plot = 0; end
if ~exist('x0_user','var'); x0_user = []; end
...
% return;
if ~run_refinement
    % package outputs before exiting
    cbf_result = struct();
    cbf_result.linear_like_form = linear_like_form;
    cbf_result.include_input_limits = include_input_limits;
    cbf_result.plant = plant;
    cbf_result.cbf_config = cbf_config;
    cbf_result.h_init_fcn = h_init_fcn;
    cbf_result.h_init_fcn1 = h_init_fcn1;
    cbf_result.X_coef = X_coef;
    cbf_result.Y_coef = Y_coef;
    cbf_result.X_fcn = X_fcn;
    cbf_result.Y_fcn = Y_fcn;
    cbf_result.h0_fcn = h0_fcn;
    cbf_result.c1_fcn = c1_fcn;
    cbf_result.c2_fcn = c2_fcn;
    return;
end
...
if exist('h_fcn','var')
    cbf_result.h_fcn = h_fcn;
    cbf_result.h_fcn1 = h_fcn1;
    cbf_result.u_fcn = u_fcn;
    cbf_result.dh_dx_fcn = dh_dx_fcn;
    cbf_result.hx = hx;
    cbf_result.ux = ux;
    cbf_result.dh_dx = dh_dx;
    cbf_result.min_eig_Qhs = min_eig_Qhs;
end
