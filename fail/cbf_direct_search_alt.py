...
Direct Search Method: This code is the workaround to the prevous failure code where u(x) is a deterministic function of the state and only h(x) is to be formualated via SOS.
The solver succeeds (since its an LMI) but the robot still crashes into the obstacle due to the input constraints
"""
...
def synthesize_fixed_u_h():
    ...
    if result.is_success():
        print("[SUCCESS] Found valid SOS h(x)")
